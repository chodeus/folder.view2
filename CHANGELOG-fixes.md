...
| # | Change | File(s) | Version |
|---|--------|---------|---------|
| 235 | Autostart tab (issue #49): new Settings → FolderView3 tab controlling the container start order globally. Three modes stored in `/boot/config/plugins/folder.view3/autostart.json`: `folder` (default — existing folder-derived sync unchanged), `custom` (the tab's drag-ordered sequence overrides folder/Docker-page order; `syncContainerOrder()` short-circuits into `fv3_apply_custom_autostart()`, so every existing conflict hook — native drag re-sort, autostart/wait toggles, Reset Order — re-asserts the custom order automatically), and `off` (FV3 never writes the autostart file, including the read-info stale prune). The sequence editor lists dockerman containers with folder badges, docker-page-style switch toggles (routed through Unraid's own `UpdateConfig.php`, then order re-applied), and per-container wait seconds. Waits live only in `unraid-autostart` — single source of truth shared with the Docker page in both directions; the tab reloads from the live file on every entry. Editor is read-only outside custom mode. Sequence names are charset-validated server-side; the custom apply keeps the #214/#231 complete-list prune guard; enabled-but-unsequenced containers append at the end, and a disabled container's sequence slot is remembered for re-enable. Full backups include `autostart.json`; import restores it and applies immediately. | `FolderView3.page`, `scripts/folderview3.js`, `styles/folderview3.css`, `server/lib.php`, `server/read_autostart.php`, `server/update_autostart.php`, 7 lang files, `README.md` | 2026.08.01 |
...
| # | Change | File(s) | Version |
|---|--------|---------|---------|
| 247 | Uninstall left `/var/log/packages/folder.view3-<version>-x86_64-1` behind, which broke every subsequent install. The remove block ran `removepkg &name;-&version;` — without the `-x86_64-1` suffix the package has carried since 2026.07.07.1 (#f61dac6 standardised the name everywhere *except* here) — so removepkg aborted with `No such package: folder.view3-<version>. Can't remove.` while the following `rm -rf` lines still deleted `&plugdir;` and the flash config. The orphaned DB entry then made `upgradepkg --install-new` report `Skipping package … (already installed)`, so reinstalling the same version never extracted the files and the plugin returned dead (plugin-manager reported success). Remove now globs `/var/log/packages/&name;-*` and `removepkg`s each basename, so it is version-agnostic and self-corrects after any future drift; `folder.view`/`folder.view2` do not match the glob. Install gained `--reinstall` so a stale entry can never suppress extraction again. Both patterns match Community Applications' own `.plg`. Users already stuck are repaired by installing this version (different package name → upgradepkg extracts; post-install prunes the stale entry). | `folder.view3.plg` | 2026.08.14 |
| 246 | `syncContainerOrder()` decided folder membership differently from `docker.js`, so the autostart file could disagree with the screen. Docker labels were missing entirely — `docker.js` assigns a container to a folder when its `folder.view3` label matches the folder name, but the PHP had no label handling, so a label-assigned container rendered inside its folder yet was ordered as unassigned. Labels are now read from `getDockerJSON("/containers/json?all=1")` (the endpoint `readInfo` already uses; `getDockerContainers()` carries no `Labels` key), and the read fails closed — an unavailable or partial Docker response aborts before the order is written rather than emitting labelled containers as unassigned. The regex gate also diverged in both directions: `empty("0")` is true in PHP, so a regex of `0` was dropped while `docker.js` applied it, and a whitespace-only regex was applied here while `docker.js` skips it. Gated on `is_string()` + `trim()` to mirror the client and to keep a non-string regex from fataling `trim()` on PHP 8. Explicit-vs-label precedence when a container is claimed by both is tracked in #55. | `server/lib.php` | 2026.08.14 |
| 245 | Import Everything rejected any file without an `fv3_export_version` wrapper, so a folder.view2 backup dead-ended on "Not a valid FV3 backup file." despite being importable via the dedicated Import Docker / Import VM buttons. It now detects a bare folder export (single folder object, or `id => folder` map) and offers Docker/VM buttons, reusing the same import path. The choice uses real buttons rather than confirm/cancel because sweetalert reports ESC and Cancel identically, and they are bound with `addEventListener` because an inline `onclick` inside sweetalert's own markup compiles but never fires. `fv3ImportFolderMap()` is extracted so the docker, vm and folder-export paths share one loop, and single-folder mode is selected with `fv3IsFolderShaped()` rather than a truthiness check on `content.name` — an empty name is legal and would otherwise post the folder's own keys as folder ids. | `scripts/folderview3.js` | 2026.08.14 |
| 244 | Organizer registry cleanup and writes are gated on a successful registry read (`regOk`) — a failed/unreachable registry read no longer risks running rename/delete cleanup against empty state or overwriting the stored registry with a freshly-adopted set. | `scripts/shared.js`, `server/lib.php`, `server/read_organizer_registry.php` | 2026.08.01 |
| 243 | Folder names `root` (case-insensitive) rejected client-side (folder.js swal) and server-side (updateFolder 400) — the name is reserved by Unraid's Docker organizer (`createFolderWithItems` throws on it), so a same-named FV3 folder could never mirror. All Autostart-tab form fields gained unique `name` attributes (`fv3-as-pos-/autostart-/wait-<container>`), clearing the DevTools "form field has neither an id nor a name" violations. | `scripts/folder.js`, `server/lib.php`, `scripts/folderview3.js` | 2026.08.01 |
| 242 | Organizer mirror: ownership registry (`organizer-registry.json` on flash) records the organizer folders FV3 created/manages; deleting or renaming an FV3 folder now deletes its organizer twin on the next sync (children return to the organizer root via `deleteDockerEntries`). Only registered folders are ever deleted — user-created native folders are untouched; pre-registry stale folders (e.g. an already-orphaned twin) are deliberately left alone. Registry adopts current FV3-named folders each pass and is only rewritten on change (no per-pageload flash writes). Sync failures now emit an unconditional `console.warn` — the previous debug-only logging is how a fully broken mirror shipped unnoticed for two weeks. Live-verified: create FV3 folder → twin created+registered; delete FV3 folder → twin removed, registry pruned. A missed create self-heals on the next page load (observed one lazy-persist race with the API). | `scripts/shared.js`, `server/lib.php`, `server/read_organizer_registry.php`, `server/update_organizer_registry.php` | 2026.08.01 |
| 241 | Autostart tab: toggle saves now re-read the live autostart file first instead of trusting the load-time snapshot. Stock `UpdateConfig.php` removes an entry by exact `"name wait"` line match — a mismatch returns `false` and `unset($arr[false])` deletes entry 0 (the wrong container), and re-adding with a different wait appends a duplicate line. If the wait changed elsewhere (docker page in another tab) between tab load and Apply, either footgun could fire. Fresh-read closes the window to milliseconds and skips toggles the concurrent edit already satisfied. | `scripts/folderview3.js` | 2026.08.01 |
| 240 | Autostart tab: jquery.switchButton fires a synthetic `change` during initialization, and the toggle handler responded by re-appending every row to its tbody — order-preserving by luck, but a latent reorder trap (surfaced when an injected test row silently moved to the bottom). The handler now no-ops unless the checked state genuinely disagrees with which section the row is in. | `scripts/folderview3.js` | 2026.08.01 |
| 239 | Autostart tab: type-to-jump — the position column is now an editable number input; committing a value (Enter or blur) moves the row to that position, clamped to the sequence length. Long-fleet ergonomics: drag for local nudges, typed positions for long moves. Apply/Reset relocated under the mode selector (right-aligned via `.fv3-as-buttons`); the plugin-wide right-alignment from #238 reverted — other panels' bars are centered again. Long container/folder names verified to ellipsize without breaking the column layout. | `FolderView3.page`, `scripts/folderview3.js`, `styles/folderview3.css` | 2026.08.01 |
| 238 | UI consistency pass: Autostart sequence table gained a live position-number column (renumbers on drop/toggle); all `.fv3-button-bar` Apply/Reset bars are right-aligned to their content column plugin-wide; the folder editor's Submit button renamed to Apply (`data-i18n` switched to the existing `apply` key) to match every other settings confirmation. | `FolderView3.page`, `Folder.page`, `scripts/folderview3.js`, `styles/folderview3.css` | 2026.08.01 |
| 237 | Autostart tab: outside Custom mode the sequence table now displays the live autostart-file order rather than the remembered custom sequence, so the tab is a true representation of the current start order in every mode. Apply/Reset moved to the top of the panel for visibility. | `scripts/folderview3.js`, `FolderView3.page` | 2026.08.01 |
| 236 | Autostart tab: sequence rows could still be dragged outside Custom mode by grabbing the container **icon** — an `<img>` is natively draggable regardless of the row's `draggable="false"`, and the drag bubbled into the row's dragstart handler. Icons now render `draggable="false"`, the dragstart handler cancels any drag from a non-draggable row, and the sorter no-ops unless a genuinely draggable row started the drag. | `scripts/folderview3.js` | 2026.08.01 |
| 234 | Dashboard: the API update-status check iterated `Object.keys(folder.containers)` — but on the Dashboard `containers` is still the name **array** from the folder config (docker.js replaces it with a name-keyed object at render; dashboard.js never does), so the keys were `"0","1","2"` and folder update badges never lit from the Dashboard's API path. Now iterates the names whichever shape is present. | `scripts/dashboard.js` | 2026.08.01 |
| 233 | Organizer mirror rewritten from `setDockerFolderChildren` to `moveDockerEntriesToFolder`: the set mutation overwrites a folder's child list **without detaching entries from their old parent**, so every populated container remained a child of the organizer root too (verified 45 duplicate references live). Move semantics detach-then-attach, reconcile removed members back to root (containers only — nested user folders are left alone), self-heal pre-existing root duplicates, and a folder whose members all fail to resolve is skipped instead of being emptied (a resolution failure is not an empty folder). | `scripts/shared.js` | 2026.08.01 |
| 232 | Unraid 7.2+ native Docker organizer sync (`fv3SyncOrganizer`) populated every mirrored folder **empty** since it shipped (2026.07.17): the API's `flatEntries` returns docker-style container names (`/plex`) which never matched FV3's slash-less names, and the Dashboard call path additionally passed `containers` as an array to an `Object.keys()` lookup. Names are now slash-normalized and both container shapes handled; verified live — all folders populate on first Docker page load. | `scripts/shared.js` | 2026.08.01 |
